    ...
    def forward(
        ...
        **kwargs,
        ...
        causal_mask = torch.full(
            (q_len, kv_seq_len), min_dtype, dtype=attn_weights.dtype, device=attn_weights.device
        )
        ...
        if attention_mask is not None:
            if attention_mask.dim() == 2:
                attention_mask = attention_mask[:, None, None, :].to(dtype=attn_weights.dtype, device=attn_weights.device)
...
class LlamaDecoderLayer(nn.Module):
    def __init__(self, config: LlamaConfig):
        super().__init__()
        self.hidden_size = config.hidden_size
        self.self_attn = LlamaFlashAttention2(config=config) if _flash_attn_available else LlamaAttention(config=config)
