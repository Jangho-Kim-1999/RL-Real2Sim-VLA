...
parser.add_argument(
    ...
    type=int,
    default=None,
    help="Stop after saving this many NaVILA annotation samples. The last episode is clipped if needed.",
)
...
parser.add_argument("--robot_radius_range", type=float, nargs=2, default=(0.6, 3.0), metavar=("MIN", "MAX"))
parser.add_argument("--target_distance_range", type=float, nargs=2, default=(0.3, 3.0), metavar=("MIN", "MAX"))
parser.add_argument(
    "--target_angle_range",
    type=float,
    nargs=2,
...
parser.add_argument("--robot_lateral_range", type=float, nargs=2, default=(-3.0, 3.0), metavar=("MIN", "MAX"))
parser.add_argument("--robot_yaw_noise_range", type=float, nargs=2, default=(-math.pi, math.pi), metavar=("MIN", "MAX"))
parser.add_argument(
    "--independent_spawn",
    action="store_true",
    help="Sample object, robot, and target positions independently instead of using relative curriculum geometry.",
)
parser.add_argument(
    ...
    nargs="+",
    metavar=("X", "Y"),
    help="Shared XY polygon for independent object/robot/target spawn: x1 y1 x2 y2 ...",
)
parser.add_argument("--object_x_range", type=float, nargs=2, default=None, metavar=("MIN", "MAX"))
parser.add_argument("--object_y_range", type=float, nargs=2, default=None, metavar=("MIN", "MAX"))
parser.add_argument("--robot_x_range", type=float, nargs=2, default=None, metavar=("MIN", "MAX"))
parser.add_argument("--robot_y_range", type=float, nargs=2, default=None, metavar=("MIN", "MAX"))
parser.add_argument("--robot_yaw_range", type=float, nargs=2, default=None, metavar=("MIN", "MAX"))
parser.add_argument("--target_x_range", type=float, nargs=2, default=None, metavar=("MIN", "MAX"))
parser.add_argument("--target_y_range", type=float, nargs=2, default=None, metavar=("MIN", "MAX"))
parser.add_argument("--target_yaw_range", type=float, nargs=2, default=None, metavar=("MIN", "MAX"))
parser.add_argument(
    "--disable_lighting_randomization",
    action="store_true",
    help="Keep scene lights fixed instead of randomizing brightness/color every episode.",
)
parser.add_argument(
    "--distant_light_intensity_range",
    type=float,
    nargs=2,
    default=(1500.0, 4500.0),
    metavar=("MIN", "MAX"),
)
parser.add_argument(
...
parser.add_argument(
    ...
    type=float,
    default=0.12,
    help="Per-channel multiplicative light color jitter. 0 disables color randomization.",
)
...
def _optional_sorted_range(values: tuple[float, float] | list[float] | None) -> tuple[float, float] | None:
    if values is None:
        return None
    return _sorted_range(values)


def _polygon_vertices_or_none() -> list[tuple[float, float]] | None:
    values = args_cli.spawn_polygon_vertices
    if values is None:
        return None
    if len(values) < 6 or len(values) % 2 != 0:
        raise ValueError("spawn_polygon_vertices must be x1 y1 x2 y2 x3 y3 ... with at least three vertices.")
    return [(float(values[i]), float(values[i + 1])) for i in range(0, len(values), 2)]


def _has_independent_spawn_override() -> bool:
    return any(
        value is not None
        for value in (
            args_cli.spawn_polygon_vertices,
            args_cli.object_x_range,
            args_cli.object_y_range,
            args_cli.robot_x_range,
            args_cli.robot_y_range,
            args_cli.robot_yaw_range,
            args_cli.target_x_range,
            args_cli.target_y_range,
            args_cli.target_yaw_range,
        )
    )
...
def _randomized_light_color(base_color: tuple[float, float, float], rng: random.Random) -> tuple[float, float, float]:
    jitter = max(0.0, float(args_cli.light_color_jitter))
    if jitter <= 0.0:
        return base_color
    return tuple(_clamp01(channel * rng.uniform(1.0 - jitter, 1.0 + jitter)) for channel in base_color)


def _set_light_prim(stage, prim_path: str, intensity: float, color: tuple[float, float, float]) -> bool:
    if stage is None:
        return False
    ...
    return intensity_attr.IsValid() or color_attr.IsValid()


def _randomize_scene_lighting(rng: random.Random) -> dict:
    stage = get_current_stage()
    distant_intensity = rng.uniform(*_sorted_range(args_cli.distant_light_intensity_range))
    dome_intensity = rng.uniform(*_sorted_range(args_cli.dome_light_intensity_range))
    distant_color = _randomized_light_color((0.85, 0.85, 0.85), rng)
    dome_color = _randomized_light_color((0.15, 0.15, 0.15), rng)

    return {
        "distant_light": {
            "prim_path": "/World/light",
            "intensity": float(distant_intensity),
            "color": list(distant_color),
            "applied": _set_light_prim(stage, "/World/light", distant_intensity, distant_color),
        },
        "dome_light": {
            "prim_path": "/World/skyLight",
            "intensity": float(dome_intensity),
            "color": list(dome_color),
            "applied": _set_light_prim(stage, "/World/skyLight", dome_intensity, dome_color),
        },
    }
...
def _apply_collector_env_overrides(env_cfg) -> None:
    """Use final-curriculum data-collection spawn ranges and prevent success auto-reset."""
    ...
    independent_spawn = bool(args_cli.independent_spawn or _has_independent_spawn_override())
    reset_params.update(
        {
            "independent_spawn": independent_spawn,
            "spawn_polygon_vertices": _polygon_vertices_or_none(),
            "object_x_range": _optional_sorted_range(args_cli.object_x_range),
            "object_y_range": _optional_sorted_range(args_cli.object_y_range),
            "robot_x_range": _optional_sorted_range(args_cli.robot_x_range),
            "robot_y_range": _optional_sorted_range(args_cli.robot_y_range),
            "robot_yaw_range": _optional_sorted_range(args_cli.robot_yaw_range),
            "target_x_range": _optional_sorted_range(args_cli.target_x_range),
            "target_y_range": _optional_sorted_range(args_cli.target_y_range),
            "target_yaw_range": _optional_sorted_range(args_cli.target_yaw_range),
...
def _clip_saved_episode_samples(
    save_dir: Path,
    episode_id: int,
    samples: list[dict],
    ...
    annotations = annotations[:keep_count]

    episode_dir = save_dir / f"episode_{int(episode_id):04d}"
    write_json(episode_dir / "samples.json", samples)
    metadata_path = episode_dir / "metadata.json"
    metadata = _load_json_if_exists(metadata_path, {})
    metadata["num_samples"] = len(samples)
    metadata["clipped_to_target_samples"] = True
...
def _collect_episode(env: PlainRslRlVecEnvWrapper, policy, episode_id: int, rng: random.Random):
    ...
    if not args_cli.disable_lighting_randomization:
        lighting_metadata = _randomize_scene_lighting(rng)
    base_env.sim.render()
    ...
    if lighting_metadata is not None:
        metadata["lighting_randomization"] = lighting_metadata
    return {
    ...
    try:
        for local_ep in range(max(0, int(args_cli.episodes))):
            ...
            if target_samples is not None:
                remaining_samples = max(0, int(target_samples) - len(all_samples))
                samples, annotations = _clip_saved_episode_samples(
                    save_dir,
                    result["episode_id"],
                    samples,
                    annotations,
            ...
            if target_samples is not None and len(all_samples) >= int(target_samples):
                print(f"[INFO] Reached target_samples={int(target_samples)}")
                break
    finally:
        env.close()
