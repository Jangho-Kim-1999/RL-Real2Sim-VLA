...
def _head_pos(env: ManagerBasedRLEnv, asset_cfg: SceneEntityCfg = SceneEntityCfg("robot")) -> torch.Tensor:
    asset: Articulation = env.scene[asset_cfg.name]
    pos_r = asset.data.root_pos_w - env.scene.env_origins
    quat_r = asset.data.root_quat_w
    head_pos = torch.tensor(HEAD_POS, device=env.device).repeat(env.num_envs, 1)
    return pos_r + math_utils.quat_apply(quat_r, head_pos)


def reach_goal_single(
    env: ManagerBasedRLEnv,
    base_height: float,
    threshold: float,
    ...
    dis = torch.norm(_head_pos(env, asset_cfg) - _single_goal_pos(env, base_height, target_cfg), dim=1)
    return (dis < threshold).float()


class goal_dis_single(ManagerTermBase):
    def __init__(self, cfg: RewardTermCfg, env: ManagerBasedRLEnv):
        super().__init__(cfg, env)
        self.dis = torch.zeros(env.num_envs, device=env.device)
...
def goal_heading_single_l1(
    ...
    asset: Articulation = env.scene[asset_cfg.name]
    root_env = asset.data.root_pos_w - env.scene.env_origins
    pos_goal = _single_goal_pos(env, base_height, target_cfg)
    ...
    yaw_error = torch.abs(goal_yaw - current_yaw)
    yaw_error = torch.where(yaw_error > torch.pi, 2 * torch.pi - yaw_error, yaw_error)
    dis = torch.norm(_head_pos(env, asset_cfg) - pos_goal, dim=1)
    return 1 - (2 * yaw_error / torch.pi) * (dis > 0.25)


def stand_still_at_single_goal(
    env: ManagerBasedRLEnv,
    base_height: float,
    asset_cfg: SceneEntityCfg = SceneEntityCfg("robot"),
    target_cfg: SceneEntityCfg = SceneEntityCfg("cone_red"),
